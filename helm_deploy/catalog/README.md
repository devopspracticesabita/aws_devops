...
# It will roll back to the previous stable version of helm upgrade gets failed.
helm upgrade <release_name> <chart> --atomic --timeout 5m

# It will roll back to the previous stable version of helm upgrade gets failed and --cleanup-on-fail to ensure that any new resources created during the failed attempt (which might not exist in the old version) are deleted
helm upgrade <release_name> <chart> --atomic --timeout 5m --cleanup-on-fail